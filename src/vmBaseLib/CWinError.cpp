...
namespace vm{
	...
	/////////////////////////////////////////////////////////////////////////////////////////
	// Name      : CWinErr
	// Full name : CWinErr::CWinErr
	// Access    : public 
	// Brief     : 
	// Parameter : const DWORD cdwError/*=GetLastError() */
	// Return    : 
	// Notes     : 
	CWinErr::CWinErr(const DWORD cdwError/*=GetLastError() */) : mdwErrorCode(cdwError)
	{
	...
	const char* CWinErr::ToString()
	{
		try
		{
			...
			// 将获得的信息
			mstrErrorDes = std::string((LPCSTR)(mszBuf)); //ssBuf.str().c_str();
	...
	/////////////////////////////////////////////////////////////////////////////////////////
	// Name      : ToCode
	// Full name : CWinErr::ToCode
	// Access    : public 
	// Brief     : 
	// Parameter : 
	// Return    : unsigned long
	// Notes     : 
	unsigned long CWinErr::ToCode()
	{
		return mdwErrorCode;
	} // End of function ToCode(...
	/////////////////////////////////////////////////////////////////////////////////////////

	/////////////////////////////////////////////////////////////////////////////////////////
	// Name      : Format
	// Full name : CWinErr::Format
	// Access    : public 
	// Brief     : 
	// Parameter : const char* const szFormat
	// Return    : const char*
	// Notes     : 
	const char* CWinErr::Format(const char* const szFormat)
	{
		_VERIFY_(szFormat);
		...
		// 计算字符串长度用于保证格式化后的字符串不能越界
		size_t lstLenForTmp = strlen(szFormat);
		size_t lstMinLen = _MIN_( sizeof(mszBuf), lstLenForTmp);

		char* lpBuf = (char*)mszBuf;
		...
		do
		{
			...
			case 'M':
			{
				memcpy(lpBuf, mstrErrorDes.c_str(), mstrErrorDes.size());
				lpBuf += mstrErrorDes.size();
				lpFormat += 1;
			}
			break;
			case 'C':
			{
				vm::CValue lValue(mdwErrorCode);
